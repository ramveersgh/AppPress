/**
 * @preserve Basic Primitives orgDiagram v2.0.8
...
primitives.common.BaseShape.prototype._getArrow = function (offsetX, offsetY, length, width, angle) {
    var result = [],
        index, len,
        point, x, y,
...
primitives.common.BaseShape.prototype._quadraticBezierPoint = function (first, control, second, time) {
...
primitives.orgdiagram.BaseController.prototype._setHighlightItem = function (event, newHighlightItem) {
    ...
    if (newHighlightItem !== null) {
        if (newHighlightItem.itemConfig.id !== this.options.highlightItem) {
            eventArgs = this._getEventArgs(this._highlightTreeItem, newHighlightItem);
...
primitives.orgdiagram.BaseController.prototype._getNextLevelTreeItem = function (newHighlightItem, isBelow) {
    ...
    for (levelIndex = 0, levelLen = nextLevels.length; levelIndex < levelLen; levelIndex += 1) {
        ...
        for (index = 0, len = treeLevelItems.length; index < len; index += 1) {
            treeItem = this._treeItems[treeLevelItems[index]];
                ...
                switch (direction) {
                    ...
                    case 2/*primitives.common.OrientationType.Left*/:
                        ...
                        for (index = currentHilightItem.levelPosition - 1; index >= 0; index -= 1) {
                            treeItem = this._treeItems[treeLevel.treeItems[index]];
                            if (treeItem.actualVisibility == 1/*primitives.common.Visibility.Normal*/) {
				...
				this, function (x, y, width, height) {
					...
					switch (treeItem.actualVisibility) {
						...
						case 2/*primitives.common.Visibility.Dot*/:
						    ...
						    segments.push(new primitives.common.DotSegment(x, y, width, height, treeItem.template.minimizedItemCornerRadius));
		    ...
		    this.transform.transformPoints(parentHorizontalCenter, partnersConnectorOffset,
				parentHorizontalCenter, childrenConnectorOffset, true, this, function (fromX, fromY, toX, toY) {
				    ...
				    if (this.showElbowDots && parentTreeItem.partners.length > 1) {
				        segments.push(new primitives.common.DotSegment(fromX, fromY, 1, 1, 0.5));
				    }
				    segments.push(new primitives.common.MoveSegment(fromX, fromY));
				    segments.push(new primitives.common.LineSegment(toX, toY));
				    if (this.showElbowDots &&  points.length > 1) {
				        segments.push(new primitives.common.DotSegment(toX, toY, 1, 1, 0.5));
				    }
				});
		}
	}
...
primitives.famdiagram.Controller.prototype._createOrgTree = function () {
    ...
    if (this.options.items.length > 0) {
        ...
        this._alignPartners(extraGravities);
    }
};

primitives.famdiagram.Controller.prototype._alignPartners = function (extraGravities) {
    var index, len,
        ...
        extraGravityItems,
        extraGravity;
    /* Partners are not used in alignment of branches, so at this stage we place partners on left or right side relative to their parents */
    for (index = 0, len = this.partnersToAlign.length; index < len; index += 1) {
        partner = this.partnersToAlign[index];
        extraGravityItems = extraGravities[partner.id];

        /* we count total gravity for our partner using items it is collected to */
        gravity = 0;
        for (index2 = 0, len2 = extraGravityItems.length; index2 < len2; index2 += 1) {
            extraGravity = extraGravityItems[index2];
            gravity += (this._orgItems[extraGravity.fromParent].childIndex < this._orgItems[extraGravity.toParent].childIndex) ? 1 : -1;
        }
        partner.adviserPlacementType = (gravity >= 0) ? 3/*primitives.common.AdviserPlacementType.Right*/ : 2/*primitives.common.AdviserPlacementType.Left*/;
    }
};
...
primitives.famdiagram.Controller.prototype._getExtraGravity = function () {
    var orgItemId, orgItem,
        ...
        index, len,
        extraChildren,
        extraChild, children, extraChildParent,
    ...
    for (orgItemId in this._orgChildren) {
        if (this._orgChildren.hasOwnProperty(orgItemId)) {
            ...
            for (index = 0, len = extraChildren.length; index < len; index += 1) {
                ...
                this._addExtraGravitiesForConnection(result, extraChildParent, orgItem, children);
            }
        }
    }
...
primitives.famdiagram.Controller.prototype._addExtraGravitiesForConnection = function (extraGravities, fromItem, toItem, children) {
    ...
    /* this collection maybe empty, contain single child item or child item and its partner*/
    for (index = 0, len = children.length; index < len; index += 1) {
        childItem = children[index];
        this._addExtraGravityForItem(extraGravities, childItem.id, extraGravityFrom);
    }
